...
namespace BusinessLogic
{
    public class DES
    {
        ...
        public string xor(string aInput, string bInput)
        {
            ...
            long t_b = Convert.ToInt64(bInput, 16);
            t_a = t_a ^ t_b;
            aInput = Convert.ToString(t_a,16);
            while (aInput.Length < bInput.Length)
                aInput = "0" + aInput;
            return aInput;
        }
        //считаем сдвиг
        ...
        public String[] getKeys(String key)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                var leftKey =RemoveWhitespace(leftCircularShift(key.Substring(0, 7), shiftBits[i]));
                var rightKey = RemoveWhitespace(leftCircularShift(key.Substring(7, 7), shiftBits[i]));
                key = leftKey + rightKey;
                var temp = permutation(PC2, key);
                keys[i] = temp;
            }
            return keys;
        }
        ...
        public void Test()
        {
            ...
            DES cipher = new DES();
            Console.WriteLine("Зашифрованные:\n");
            text = cipher.encrypt(text, key);
            Console.WriteLine(
                "\nТекст:" + text.ToUpper() + "\n");
            Console.WriteLine("Расшифрованные\n");
